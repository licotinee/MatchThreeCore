        ...
        public void HandleInput()
        {
            ...
            if (drag && selectedTile != null)
            {
                ...
                if (hit.collider != null && hit.collider.gameObject != selectedTile)
                {
                    ...
                    if (combo != null)
                    {
                        GameController.Instant.CurrentPlayerAction(null,true);
...
//                    SoundManager.instance.PlaySound("Chocolate");
        ...
        private bool HandleMatches(bool isPlayerMatch)
        {
            ...
            if (matches.Count > 0)
            {
                ...
                foreach (Match match in matches)
                {
                    ...
                    GameController.Instant.CurrentPlayerAction(match.tiles);
            ...
            {
                GameController.Instant.CurrentPlayerAction(null);
                return false;
            }
        }
        ...
        private bool CheckCollectables()
        {
            ...
            {
                foreach (Tile tile in collectablesToDestroy)
                {
                    gameState.AddCollectable(tile.GetComponent<Collectable>().type);
                    
                    UpdateScore(gameConfig.GetTileScore(tile));
